...
function sumTwoSmallestNumbers(numbers) {
  ...
  for (let i = 0; i < numbers.length; i++) {
    if (lowStack.length < 1) {
      lowStack.push(numbers[0]);
    }
    else if (lowStack.length === 1) {
      if (numbers[1] < lowStack[0]) {
        lowStack.unshift(numbers[1]);
      }
      else {
        lowStack.push(numbers[1]);
      }
    }
    else {
      if (numbers[i] < lowStack[0]) {
        lowStack.pop();
        lowStack.unshift(numbers[i]);
      }
      else if (numbers[i] < lowStack[1]) {
        lowStack.pop();
        lowStack.push(numbers[i]);
      }
    }
  }

  return lowStack[0] + lowStack[1];
}
